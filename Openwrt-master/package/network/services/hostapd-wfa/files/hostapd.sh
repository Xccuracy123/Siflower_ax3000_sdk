...
hostapd_prepare_device_config() {
	...
	json_get_vars country country3 country_ie beacon_int:100 dtim_period:1 doth require_mode legacy_rates \
		acs_chan_bias rd_disabled local_pwr_constraint spectrum_mgmt_required airtime_mode cell_density \
...
hostapd_common_add_bss_config() {
	...
	config_add_boolean wds wmm uapsd hidden utf8_ssid cond_hiddenu

	config_add_int maxassoc max_inactivity band_steering
	...
	config_add_boolean transition_disable
}
...
hostapd_set_bss_options() {
	...
	json_get_vars \
		...
		bss_load_update_period chan_util_avg_period sae_require_mfp sae_pwe sae_groups\
		multi_ap multi_ap_backhaul_ssid multi_ap_backhaul_key skip_inactivity_poll \
		wpa_group_update_count band_steering \
		airtime_bss_weight airtime_bss_limit airtime_sta_weight \
		multicast_to_unicast proxy_arp per_sta_vif \
		eap_server eap_user_file ca_cert server_cert private_key private_key_passwd server_id \
		vendor_elements fils transition_disable
	...
	set_default band_steering 0
	set_default airtime_bss_weight 0
	set_default airtime_bss_limit 0
	set_default eap_server 0
	set_default transition_disable 1
	...
	append bss_conf "band_steering=$band_steering" "$N"
...
wpa_supplicant_add_network() {
	...
	json_get_vars \
		...
		ieee80211w ieee80211r fils disable_network \
		multi_ap \
		default_disabled
	...
	case "$auth_type" in
		...
		wep)
			...
			if [ "$auth_mode_shared" = "1" -a "$auth_mode_open" = "0" ]; then
