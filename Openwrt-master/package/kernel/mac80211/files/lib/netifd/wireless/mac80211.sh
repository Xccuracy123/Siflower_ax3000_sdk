...
# [ "$hwmode" = "a" ] || enable_ac=0
	...
	if [ "$enable_ax" != "0" ]; then
		...
		[ -n "$he_bss_color" ] && [ "$he_bss_color" -ge 0 ] && [ "$he_bss_color" -le 63 ] && {
		...
		[ -n "$he_er_su_disable" ] && {
			append base_cfg "he_er_su_disable=$he_er_su_disable" "$N"
		}
...
mac80211_vap_cleanup() {
	...
	for wdev in $vaps; do
#		Commented out here to prevent instability when restart wifi  @bug #1002522
...
mac80211_rm_conf_file() {
    ...
    case "$mode" in
        ap)
            [ -f /var/run/hostapd-${phy}.conf ] && rm /var/run/hostapd-${phy}.conf
        ;;
        sta)
...
unused
	...
	[ -n "$txpower_lvl" ] && {
                if [ "$band" == "2.4G" ]; then
			if [ -d  "/sys/module/siwifi_lb_fmac" ]; then
				echo $txpower_lvl > /sys/module/siwifi_lb_fmac/parameters/txpower_lvl
			fi
                else
			if [ -d  "/sys/module/siwifi_hb_fmac" ]; then
				echo $txpower_lvl > /sys/module/siwifi_hb_fmac/parameters/txpower_lvl
			fi
                fi
        }
...
#					for_each_interface "sta adhoc mesh monitor" mac80211_prepare_vif
		...
		if [ "$no_reload" != "0" ]; then
			add_ap=1
			wireless_set_setup_pid_file "/var/run/setup-wifi-$phy.pid"
			/usr/sbin/hostapd -s -P /var/run/wifi-$phy.pid -B "$hostapd_conf_file"
			local hostapd_res="$(cat /var/run/wifi-$phy.pid)"
			ret="$?"
