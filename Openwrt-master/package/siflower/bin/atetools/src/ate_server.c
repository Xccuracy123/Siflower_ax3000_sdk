...
#define CALI_TABLE_5G_SIZE 2525
...
#define SFCFG_CMD_ATE_TEMP_GET      0x10001
...
uint32_t tmp_mib_20m = 0;
uint32_t tmp_mib_40m = 0;
uint32_t tmp_mib_80m = 0;
uint32_t tmp_mib_160m = 0;
...
int32_t get_temp_info(void)
{
       ...
       tmp.magic_no = SFCFG_MAGIC_NO;
       tmp.comand_type = SFCFG_CMD_ATE_TEMP_GET;
       tmp.comand_id   = SFCFG_CMD_ATE_TEMP_GET;
       tmp.length      = 10;
       tmp.sequence    = 1;
...
int32_t ant_num_set(char *tmpdata)
{
	...
	memcpy(tmp.data,tmpdata, 10);
	printf("tmp.data= %s -----ant-----\n", tmp.data);

	wrq.u.data.pointer = (caddr_t)&tmp;
	wrq.u.data.length = sizeof(struct sfcfghdr) - sizeof(((struct sfcfghdr *)0)->data) + tmp.length;

	if (do_ioctl(SFCFG_PRIV_IOCTL_ATE, &wrq) < 0) {
		printf("SFCFG_PRIV_IOCTL_ATE ioctl SFCFG_CMD_ATE_ANT_NUM failed\n");
		return -1;
	}
	return 0;
}
...
int32_t get_rx_info(uint8_t flag)
{
	...
	tmp_mib_20m = user->mib_20m;
	tmp_mib_40m = user->mib_40m;
	tmp_mib_80m = user->mib_80m;
	tmp_mib_160m = user->mib_160m;
...
int32_t save_power_table_to_factory(uint32_t length,uint32_t offset,char *buffer){
	...
	printf("cali_table[0] :%d \n",buffer[0]);
	printf("cali_table[3201] :%d \n",buffer[3201]);
	...
	flash_write(buffer, offset_tmp + 2048 + 4, length);
	flash_deinit();
#else
	if (do_ioctl(SFCFG_PRIV_IOCTL_ATE,&wrq) < 0) {
...
//	uint32_t sizeof_buffer = 0;
	...
	flash_write(buffer, offset, data_length);
	flash_deinit();
#else
	if (do_ioctl(SFCFG_PRIV_IOCTL_ATE,&wrq) < 0) {
		printf("SFCFG_PRIV_IOCTL_ATE ioctl SFCFG_CMD_ATE_SAVE_DATA_TO_FLASH failed\n");
...
int32_t save_power_config_to_flash(uint16_t tmpdata, uint16_t channel, uint16_t bw, uint16_t mode, uint16_t rate)
{
	...
	flash_write(&num, offset + 2048 + 4, 1);
	flash_deinit();
#else
...
#else
	wrq.u.data.pointer = (caddr_t)&tmp;
	wrq.u.data.length = sizeof(struct sfcfghdr) - sizeof(((struct sfcfghdr *)0)->data) + 6;
...
int cali_rssi_offset(char *band, char *tmpdata)
{
	...
	if(abs(rssi_offset1) > 20 || abs(rssi_offset2) > 20) {
		printf("rssi offset is %d and %d, which are too large, something may went error\n", rssi_offset1, rssi_offset2);
		return -1;
	}
...
int32_t ate_main(char *buffer)
{
	...
	//**************************************
		...
		if (!strncasecmp(buffer, "ate_cmd fastconfig z ", 20)) {
			...
			sscanf(buffer, "ate_cmd fastconfig %s f:%s c:%s w:%s m:%s i:%s p:%s N:%s B:%s", \
				op, pri_freq, cen_freq, rf_bw, mode, mcs, power, nss, band);
		...
		if (g_rx_frame_flag) {
			...
			if (set_phy_bandwidth(rf_bw))
				return -2;
		} else if (g_start_tx_frame_by_macbypass_flag || g_start_tx_rx_by_macbypass_flag) {
...
NEXT:
		...
		if (g_start_tx_rx_by_macbypass_flag) {
			if(send_tx_rx_start_by_macbypass())
				return -2;
			g_start_tx_rx_by_macbypass_flag = false;
			goto DONE;
		}
		...
		if (g_stop_tx_rx_by_macbypass_flag) {
			if(send_tx_rx_stop_by_macbypass())
				return -2;
			g_stop_tx_rx_by_macbypass_flag = false;
			goto DONE;
		}
...
int main()
{
...
#if USE_ONE_CALI_TABLE
	char bufsend_calitable[2048] = {0};
	char buffer2[2048] = {0};
#else
	char bufsend_calitable[6406] = {0};
	char buffer2[6406] = {0};
	char tmp_buffer2[6406] = {0};
	char tmp_buff_lb1[676] = {0};
	char tmp_buff_lb2[676] = {0};
	char tmp_buff_hb1[2525] = {0};
	char tmp_buff_hb2[2525] = {0};
	char buffer_lb1[678] = {0};
	char buffer_lb2[678] = {0};
	char buffer_hb1[2527] = {0};
	char buffer_hb2[2527] = {0};
#endif
	...
	s_add.sin_addr.s_addr = inet_addr("192.168.1.1");
	s_add.sin_port = htons(portnum);
	//socket_set_option($socket, SOL_SOCKET, SO_REUSEADDR, 1);
	if (-1 == bind(sfp, (struct sockaddr *)(&s_add), sizeof(struct sockaddr))) {
		printf("bind fail !\r\n");
		printf("1.Please make sure that br-lan is 192.168.1.1!\n");
		return -1;
	}
...
BEGAIN:
			...
			if (cali_table_write_flg == 1) {
				...
				goto BEGAIN;
#else
				// stop recv until receive the entire calibration table
				printf("waiting table from tool...\n");
				for (i = 0; i < 4; i++) {
					recv_len = 0;
					if (i == 0) {
						recv_len += recv(nfp, tmp_buff_lb1, sizeof(tmp_buff_lb1), 0);
						strncpy(bufsend, "recv lb1 table ok!", 18);
						write(nfp, bufsend, strlen(bufsend));
						strncpy(buffer_lb1, tmp_buff_lb1, 676);
						strcat(buffer2, buffer_lb1);
					} else if (i == 1) {
						do {
							recv_len += recv(nfp, tmp_buff_hb1, sizeof(tmp_buff_hb1), 0);
							strncpy(bufsend, "recv hb1 table ok!", 18);
							write(nfp, bufsend, strlen(bufsend));
							strcat(buffer_hb1, tmp_buff_hb1);
						} while (recv_len <= CALI_TABLE_5G_SIZE);
						strncpy(buffer2 + CALI_TABLE_2G_SIZE, buffer_hb1, CALI_TABLE_5G_SIZE);
					} else if (i == 2) {
						do {
							recv_len += recv(nfp, tmp_buff_lb2, sizeof(tmp_buff_lb2), 0);
							strncpy(bufsend, "recv lb2 table ok!", 18);
							write(nfp, bufsend, strlen(bufsend));
							strcat(buffer_lb2, tmp_buff_lb2);
						} while (recv_len <= CALI_TABLE_2G_SIZE);
						strncpy(buffer2 + CALI_TABLE_2G_SIZE + CALI_TABLE_5G_SIZE, buffer_lb2, CALI_TABLE_2G_SIZE);
					} else if (i == 3) {
						do {
							recv_len += recv(nfp, tmp_buff_hb2, sizeof(tmp_buff_hb2), 0);
							strncpy(bufsend, "recv hb2 table ok!", 18);
							write(nfp, bufsend, strlen(bufsend));
							strcat(buffer2, tmp_buff_hb2);
						} while (recv_len <= CALI_TABLE_5G_SIZE);
					}
				}
				printf("get table from tool over\n");

				memset(bufsend_calitable, 0, 6406);
				memcpy(bufsend_calitable, buffer2, 6406);
#if 1
				...
				for (i = 0; i < 25; i++) {
					if (i < 16) {
						...
						for (j = 0; j < 101; j++) {
							if (j >= 89 && j < 101) {
								bufsend_calitable[CALI_TABLE_2G_SIZE + i * 101 + j] = 0;
								bufsend_calitable[CALI_TABLE_2G_SIZE * 2 + CALI_TABLE_5G_SIZE + i * 101 + j] = 0;
							}
						}
					} else if (i == 24) {
				...
				for (i = 0; i < 6402; i++) {
					//TODO need write xo & version
					if (i >= 0 && i < CALI_TABLE_2G_SIZE) {
						printf("%4d", bufsend_calitable[i]);
						// 2.4G ant1. There are 52 gains in each row.
						if ((i + 1 - 0) % 52 == 0)
							printf("\n");
					} else if (i >= CALI_TABLE_2G_SIZE && i < (CALI_TABLE_2G_SIZE + CALI_TABLE_5G_SIZE)) {
						printf("%4d", bufsend_calitable[i]);
						// 5G ant1. There are 101 gains in each row.
						if ((i + 1 - CALI_TABLE_2G_SIZE) % 101 == 0)
							printf("\n");
					} else if (i >= (CALI_TABLE_2G_SIZE + CALI_TABLE_5G_SIZE) && i < (CALI_TABLE_2G_SIZE * 2 + CALI_TABLE_5G_SIZE)) {
						printf("%4d", bufsend_calitable[i]);
						// 2.4G ant2. There are 52 gains in each row.
						if ((i + 1 - (CALI_TABLE_2G_SIZE + CALI_TABLE_5G_SIZE)) % 52 == 0)
							printf("\n");
					} else if (i >= (CALI_TABLE_2G_SIZE * 2 + CALI_TABLE_5G_SIZE) && i < 6402) {
						printf("%4d", bufsend_calitable[i]);
						// 5G ant2. There are 101 gains in each row.
						if ((i + 1 - (CALI_TABLE_2G_SIZE * 2 + CALI_TABLE_5G_SIZE)) % 101 == 0)
							printf("\n");
					}
				}
#endif
				save_power_table_to_factory(6406, 0, bufsend_calitable);
				cali_table_write_flg = 0;
				goto BEGAIN;
			}
#endif
			...
			if (!strncasecmp(buffer, "hb ate_init", 11)) {
				printf("******ate tool init hb******\n");
				strcpy(ifname, "wlan0");
				if (ant_num_set("3"))
					ret = -4;
				ret = 2;
			} else if (!strncasecmp(buffer, "lb ate_init", 11)) {
				printf("******ate tool init lb******\n");
				strcpy(ifname, "wlan1");
				if (ant_num_set("3"))
					ret = -4;
				ret = 2;
			} else if (!strncasecmp(buffer, "ate_init lb1", 12)) {
				...
				snprintf(bufsend, 1023, "%d,%d,%d,%d,%d,%d\n", user->mib_20m, user->mib_40m, user->mib_80m, user->mib_160m, user->rssi_1, user->rssi_2);
				// if (rx_pkt_all != 0) {
				...
				snprintf(bufsend, 1023, "%d,%d,%d,%d\n", user->mib_20m, user->mib_40m, user->mib_80m, user->mib_160m);
				if (-1 == write(nfp, bufsend, strlen(bufsend))) {
					printf("send cmd fail!\r\n");
					goto NEXT;
				}
				goto BEGAIN;
			} else if (!strncasecmp(buffer, "ate_cmd fastconfig rf temp", 26)) {
				memset(bufsend, 0, 1024);
				get_temp_info();
				snprintf(bufsend, 1023, "%d\n", user->rf_temp);
				if (-1 == write(nfp, bufsend, strlen(bufsend))) {
					printf("send cmd fail!\r\n");
					goto NEXT;
				}
				...
				int size = 2048;
				int n_pos = size - pos;
				printf("pos is %d, n_pos is %d, size is %d\r\n", pos, n_pos, size);
				while (n_pos > 0) {
					pos = write(nfp, &bufsend_calitable[pos], n_pos);
					n_pos = n_pos - pos;
					printf("pos is %d, n_pos is %d\r\n", pos, n_pos);
				...
				goto BEGAIN;
			} else if (!strncasecmp(buffer, "ate_cmd cali rssi", 17)) {
				sscanf(buffer, "ate_cmd cali rssi %s %s", band, tmpdata);
				cali_rssi_offset(band, tmpdata);
				ret = 2;
