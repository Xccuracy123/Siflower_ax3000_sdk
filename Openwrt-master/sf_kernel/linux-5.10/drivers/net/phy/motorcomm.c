...
static int yt8614_led_init(struct phy_device *phydev)
{
	...
	/* led init */
	/* port 3 led 1 */
	ret = ytphy_write_ext(phydev, 0xa077, 0x31);
	if (ret < 0)
		return ret;

	/* port 3 led 0 */
	ret = ytphy_write_ext(phydev, 0xa076, 0x30);
	if (ret < 0)
		return ret;

	/* port 2 led 1 */
	ret = ytphy_write_ext(phydev, 0xa075, 0x21);
	if (ret < 0)
		return ret;

	/* port 2 led 0 */
	ret = ytphy_write_ext(phydev, 0xa074, 0x20);
	if (ret < 0)
		return ret;

	/* port 1 led 1 */
	ret = ytphy_write_ext(phydev, 0xa073, 0x11);
	if (ret < 0)
		return ret;

	/* port 1 led 0 */
	ret = ytphy_write_ext(phydev, 0xa072, 0x10);
	if (ret < 0)
		return ret;

	/* port 0 led 1 */
	ret = ytphy_write_ext(phydev, 0xa071, 0x1);
	if (ret < 0)
		return ret;

	/* port 0 led 0 */
	ret = ytphy_write_ext(phydev, 0xa070, 0x0);
	if (ret < 0)
		return ret;
