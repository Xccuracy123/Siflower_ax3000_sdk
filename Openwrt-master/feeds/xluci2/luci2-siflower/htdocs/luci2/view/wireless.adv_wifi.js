L.ui.view.extend({
	execute: function() {
        ...
        var s1 = m.section(L.cbi.NamedSection, 'radio0', {
			caption:      L.tr('2.4G'),
			teasers:	  [ '_hidden', 'txpower', 'channel', '_hwmode', '_htmode' ]
		});
		...
		e1 = s1.option(L.cbi.ListValue, 'channel', {
		...
		var e3 = s1.option(L.cbi.ListValue, '_htmode', {
	    ...
	    var s2 = m.section(L.cbi.NamedSection, 'radio1', {
			caption:      L.tr('5G'),
			teasers:      [ '_hidden', 'txpower', 'channel', '_hwmode', '_htmode1', '_htmode2' ]
		});

        var e4 = s2.option(L.cbi.CheckboxValue,'_hidden',{
            caption:    L.tr('Start Broadcast')
        });
		...
		var e5 = s2.option(L.cbi.ListValue, 'txpower', {
			caption:	L.tr('Bitrate'),
			initial:	'25'
		})
        .value('25',L.tr('high'))
		...
		e5 = s2.option(L.cbi.ListValue, 'channel', {
			caption:	L.tr('Channel'),
			initial:	'161'
		})
        .value('auto',L.tr('Automatic'))
		...
		var e6 = s2.option(L.cbi.ListValue, '_hwmode', {
			caption:	L.tr('Mode'),
			initial:	'11ac'
		})
		...
		var e7 = s2.option(L.cbi.ListValue, '_htmode1', {
			caption:	L.tr('Band Width'),
		})
        ...
        var e8 = s2.option(L.cbi.ListValue, '_htmode2', {
			caption:	L.tr('Band Width'),
		})
        .value('HT20',L.tr('20MHZ'))
		.value('HT40', L.tr('40MHZ'))
		...
		if ($('#field_wireless_radio1_radio1_channel').val() == '165'){
			...
			$('#field_wireless_radio1_radio1_channel').click(function () {
				if ($('#field_wireless_radio1_radio1_channel').val() == '165'){
					$('#field_wireless_radio1_radio1__htmode2').val('HT20');
					$('#field_wireless_radio1_radio1__htmode1').val('VHT20');
					$('#field_wireless_radio1_radio1__htmode2').find('[value="HT40"]').css('display', 'none');
