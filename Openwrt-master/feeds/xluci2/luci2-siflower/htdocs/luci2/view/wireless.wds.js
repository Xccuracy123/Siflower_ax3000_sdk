L.ui.view.extend({
	...
	wifiConnect: L.rpc.declare({
		...
		params: ['device', 'ssid', 'bssid', 'encryption', 'key', 'channel', 'oc']
	}),
	...
	execute: function () {
		...
		$('body').append(loadbox(L.tr("Confirm wds setting, please wait")));
		...
		$('#wds_en').click(function () {
			if ($('#wds_en').prop('class') == 'switch_on') {
				...
				$('body').append(loadbox(L.tr("Disable wds, please wait")));
				var count = setInterval(function () { }, 10000);
				for (var cl = 1; cl <= count; cl++) {
					clearInterval(cl);
				}
				self.wds_Disable(L.uci.get('wireless', 'wds', 'device'), L.uci.get('wireless', L.uci.get('wireless', 'wds', 'device'), 'origin_channel')).then(function () {
					setTimeout(function () {
								...
								$('<div />')
									...
									.append(
										$('<div />')
											...
											.text(L.tr("Click Continue to start connecting, which may interrupt the connection")),
									),
							)
							.append(
								$('<div />')
				...
				if (list) {
					...
					self.getWifiList('wlan1').then(function (list1) {
						...
						if (checkWifi()) {
							...
							$('#password_input').val(L.uci.get('wireless', 'wds', 'key'));
		...
		function connect(info) {
														...
														if ($('#password_input').val().length < 8) {
															$('#password_input').after(
																$('<div />')
																	...
																	.text(L.tr("Length to short! Please enter at least 8 characters")),
															);
															e = false;
														} else e = true;
													} else e = true;
			...
			if ($(info).find('.encryption').text() == 'Open') {
				key_in.find('#password_input').attr('readonly', 'readonly');
			}
			$('#infobox').remove();
            $('#infoboxxx').remove();
			...
			$('#conn_wifi').click(function () {
				...
				p2 = L.uci.get('wireless', p1, 'channel');

				self.wifiConnect('radio' + $(info).prop('device'), $(info).find('.ssid').text(), $(info).prop('bssid'), $(info).find('.encryption').text(), $('#password_input').val(), String($(info).prop('channel')), L.uci.get('wireless', 'radio' + $(info).prop('device'), 'channel'));
		...
		function checkWificon(info) {
			self.wifi_status(L.globals.ifname).then(function (data) {
				y++;
				if(data != 'Unknown'){
					if(en_count == 0) {
						en_count = 1;
						...
						clearInterval(ifwi1);
						$('body').find('.loading').remove();
						$('body').append(loadbox(L.tr("Connect success, ready to start wds. Please refresh page two minutes later. If you cannot return the now page means the wds connect success.")));
														...
														if ( $('#ssid_input').val().length < 1 ) {
															$('#ssid_input').after(
																$('<div />')
																	...
																	.text(L.tr("Length is error! Please enter 1~20 characters")),
															);
															flag_ssid = false;
														} else if( $('#ssid_input').val().length > 20){
															$('#ssid_input').after(
																$('<div />')
																	.attr('id', 't3')
																	.css({
