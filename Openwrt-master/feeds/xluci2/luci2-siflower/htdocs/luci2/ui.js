(function() {
	var ui_class = {
		...
		login: function(invalid)
		{
			...
			var login_file = "login.htm";
			if(L.g_language == "en")
				login_file = "login_en.htm"

			$.ajax(L.globals.resource + '/template/' + login_file, {
				cache:    true,
				dataType: 'text',
				success:  function(data) {
...
//					L.ui.loading(false);
		...
		loadInternet: function(){
			...
			function check(v) {
				...
				if (L.parseIPv4(v)) {
					if (v.split('.')[2] == $('#field_network_wan_wan_ipaddr').val().split('.')[2]) return true;
			...
			var dns1 = s.option(L.cbi.InputValue, '__dns1', {
				...
				return v || p.formvalue('wan') == "static";
			});
			...
			dns1.ucivalue = function(sid) {
				...
				if (dns_value) {
					if (dns_value.indexOf(" ") == -1) return dns_value;
					else return dns_value.substring(0, str);
				}
			};
			...
			dns2.ucivalue = function(sid) {
				var dns_value = L.uci.get("network", "wan", "dns");
				if (dns_value) {
					if (dns_value.indexOf(" ") != -1) return dns_value.substring(str + 1);
				}
			};
			...
			pmtu1.save = function(sid) {
				...
				if(proto != 'pppoe') {
					if (pamtu > 38 && pamtu < 9000) L.uci.set('network', 'wan', 'mtu', parseInt(pamtu) + 4);
					else L.uci.set('network', 'wan', 'mtu', 1504);
					L.uci.save();
				}
			};
			...
			pmtu2.save = function(sid) {
				...
				if(proto == 'pppoe') {
					if (pamtu > 38 && pamtu < 9000)
						L.uci.set('network', 'wan', 'mtu', parseInt(pamtu) + 4);
					else
						L.uci.set('network', 'wan', 'mtu', 1484);
					L.uci.save();
				}
			};
		...
		loadTheme: function() {
			...
			self.renderMainMenu().then(function () {
				...
				var theme_file = "theme.htm";
				if(L.g_language == "en")
					theme_file = "theme_en.htm";

				$.ajax(L.globals.resource + '/template/' + theme_file, {
					cache: true,
					dataType: 'text',
					success: function (data) {
						$('body').empty().append(data);
